...
# Import the export_to_drive function from exportfolder.py
from exportfolder import export_to_drive
...
CLIENT_SECRET_FILE = 'path/to/your/client_secret.json'  # Update this path
...
def analyze_csv(data, stream=False):
    ...
    prompt = f"""Analyze the provided keywords and generate blog titles that are seasonally relevant for {__import__('datetime').datetime.now().strftime('%B %Y')}.
    Rank and align these titles by how well they support Armarkat's goals as outlined above;
    STRICT OUTPUT FORMAT:
    Return a JSON array of objects with exactly these fields:
    [{{
        "keyword": <original keyword>,
        "title": <seasonally relevant blog title>,
        "searchVolume": <monthly US volume>,
        "competition": <KD value>,
        "alignment_with_goals": <brief description of how this aligns with business goals>,
        "totalScore": <integer 1-100 based on volume, competition, and alignment>
    }}]

    REQUIREMENTS:
    - Output must be valid JSON only, no additional text
    - Sort by totalScore in descending order
    - Ensure titles are contextually relevant to the current season/holidays
    - Include all provided keywords
    - Do not add fields not specified above
    ...
    completion = client.chat.completions.create(
        model="o1-mini-2024-09-12",  # Use an appropriate model
        messages=messages
    )
...
def handle_export_to_drive():
    # This route now just calls the export_to_drive function from exportfolder.py
